...
fn main() {
    ...
    {
        topic_analyzer.add_metric_handler(&metrics);
    ...
    println!("Estimated Msg/s: {}", (metrics.overall_count() / duration_secs));
    println!("{}", "-".repeat(120));
    println!("Earliest Message: {}", metrics.earliest_message());
    println!("Latest Message: {}", metrics.latest_message());
    println!("{}", "-".repeat(120));
    println!("Largest Message: {} bytes", metrics.largest_message());
    println!("Smallest Message: {} bytes", metrics.smallest_message());
    println!("Topic Size: {} bytes", metrics.overall_size());
...
//    }
    ...
    for partition in partitions {
        ...
        table.add_row(Row::new(vec![
            ...
            Cell::new(format!("{}", metrics.total(partition)).as_str()), // Total
            Cell::new(format!("{}", metrics.alive(partition)).as_str()), // Alive
            Cell::new(format!("{}", metrics.tombstones(partition)).as_str()), // TB
            Cell::new(format!("{0:.4}", metrics.dirty_ratio(partition)).as_str()), // DR
            Cell::new(format!("{}", metrics.key_null(partition)).as_str()), // K Null
            Cell::new(format!("{}", metrics.key_non_null(partition)).as_str()), // K !Null
            Cell::new(format!("{}", metrics.key_size_sum(partition) + metrics.value_size_sum(partition)).as_str()), // P-Bytes
            Cell::new(format!("{}", metrics.key_size_sum(partition)).as_str()), // K-Bytes
            Cell::new(format!("{}", metrics.value_size_sum(partition)).as_str()), // V-Bytes
            Cell::new(format!("{}", key_size_avg).as_str()), // A-Key-Size
            Cell::new(format!("{}", metrics.value_size_avg(partition)).as_str()), // A-V-Size
            Cell::new(format!("{}", metrics.message_size_avg(partition)).as_str()), // A-M-Size
        ]));
    }
